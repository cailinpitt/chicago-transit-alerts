...
// Plain-English narrative for a pure bot observation — the "Per bot"
// counterpart to a CTA alert's `short_description`. Composes the
// observation's signal mix into a single sentence so a reader gets the
// "what's going on" without parsing chips. Returns null when the incident
// is an alert, a merged record, or carries no recognizable signals; the
// caller hides the section in that case.
//
// Why a sentence (not just chips): the existing Signals / Detection rows
// already enumerate the signals and evidence numerically. The narrative
// adds the missing piece — what the combination *means* — in the same
// register the "Per CTA" blockquote uses for official alerts.
/**
 * @param {object} incident An Observation (or compatible shape).
 * @returns {string | null}
 */
export function describeBotObservation(incident) {
  if (!incident) return null;
  // Alerts and merged records are described by `short_description`; bail so
  // we don't double up with the Per CTA blockquote.
  if (incident._type === 'merged') return null;
  if (incident.alert_id) return null;

  const signals = observationSignals(incident);
  // Filter to signals we have phrasing for. Unknown future signal kinds
...
function signalPhrase(signal, kind) {
  // Train-only signals first; bus equivalents below. pulse-* never fires on
  // bus and thin-gap never fires on train, so a phrase keyed to the wrong
  // mode is unreachable in practice.
  switch (signal) {
    case 'gap':
      return kind === 'bus'
...
function botObservationSubject(incident) {
  ...
  const label = TRAIN_LINES[lineKey]?.label;
